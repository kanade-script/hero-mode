...
local LaserOn = false
local LeftLaser, RightLaser = nil, nil
local LaserFolder = Instance.new("Folder", workspace)
LaserFolder.Name = "EyeLasers"

local function CreateBeam(StartAttachment)
	local part = Instance.new("Part")
	part.Size = Vector3.new(0.1, 0.1, 1000)
	part.Anchored = true
...
local function EnableLasers()
	if LaserOn then return end
	LaserOn = true

	local head = Character:WaitForChild("Head")

	local LeftAttachment = Instance.new("Attachment", head)
	LeftAttachment.Position = Vector3.new(-0.3, 0.2, -0.5)

	local RightAttachment = Instance.new("Attachment", head)
	RightAttachment.Position = Vector3.new(0.3, 0.2, -0.5)

	LeftLaser = CreateBeam(LeftAttachment)
	RightLaser = CreateBeam(RightAttachment)

	task.spawn(function()
		while LaserOn do
			local mouseHit = Mouse.Hit.Position
			local headPos = head.Position
			local dir = (mouseHit - headPos).Unit * 1000
			local hitPos = headPos + dir

			for _, laser in pairs({LeftLaser, RightLaser}) do
				laser.Part.CFrame = CFrame.new(head.Position, hitPos)
				laser.Attachment.WorldPosition = hitPos
			end

			local ray = workspace:Raycast(head.Position, dir, RaycastParams.new())
			if ray and ray.Instance and ray.Instance.Parent:FindFirstChild("Humanoid") then
				ray.Instance.Parent.Humanoid:TakeDamage(5)
			end
...
local function DisableLasers()
	LaserOn = false
	if LeftLaser then LeftLaser.Part:Destroy() end
	if RightLaser then RightLaser.Part:Destroy() end
	LeftLaser, RightLaser = nil, nil
end
